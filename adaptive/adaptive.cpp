...
int main(int argc, const char* argv[]){
    ...
    //std::string trees_filepath=it->second;
    ...
    for(int i=0;i<trees_number;i++){
       ...
       // std::cout<<"**************"<<"tree: "<<i+1<<" size: "<<tree_size<<"**************"<<std::endl;
           ...
           for(int j=1;j<=tree_size;j++){
               ...
               bool found=0;
               int steps=0;
              adaptive_interleave::Adaptive_interleave(target_node,trees_collection[i], trees_oracle[i], supernodes_collection[i], found, steps,thre);
